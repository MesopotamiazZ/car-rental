        ...
        <>
          ...
          <a
            onClick={() => {
              ...
              handleRemove(selectedIds,actionRef,NamespaceEnum.SysUser);
            }}
          >
            删除
          </a>
    ...
    <PageHeaderWrapper>
      <ProTable<SysUser>
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <OBtnUpload url={constructImportUrl(NamespaceEnum.SysUser)}/>,
            ...
            overlay={
              <Menu
                onClick={async e => {
                  ...
                  if (e.key === 'delete') {
                    await handleRemove(selectedIds,actionRef,NamespaceEnum.SysUser);
                    action.reload();
                  }
                  else if (e.key === 'export') {
                    await handleExport(NamespaceEnum.SysUser,selectedIds);
                  }
                  else if (e.key === 'exportAll') {
                    await handleExport(NamespaceEnum.SysUser);
                  }
                }}
                selectedKeys={[]}
              >
        ...
        params={params}
        request={params => {
                	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	                	                	        	                        	                	        	                        	        	                        	        	                          return retrievePageList(params, NamespaceEnum.SysUser);
        }}
        columns={columns}
      ...
      <CreateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleAdd(value,NamespaceEnum.SysUser);
          setSubmitting(false);
          if (success) {
      ...
      <UpdateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleUpdate(value,NamespaceEnum.SysUser);
          setSubmitting(false);
          if (success) {
