...
import SysRoleColumns from './Columns';
...
const SysRoleTableList: React.FC<{}> = () => {
        ...
        <>
          ...
          <a
            onClick={() => {
              ...
              handleRemove(selectedIds,actionRef,NamespaceEnum.SysRole);
            }}
          >
            删除
          </a>
    ...
    <PageHeaderWrapper>
      <ProTable<SysRole>
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <OBtnUpload url={constructImportUrl(NamespaceEnum.SysRole)}/>,
            ...
            overlay={
              <Menu
                onClick={async e => {
                  ...
                  if (e.key === 'delete') {
                    await handleRemove(selectedIds,actionRef,NamespaceEnum.SysRole);
                    action.reload();
                  }
                  else if (e.key === 'export') {
                    await handleExport(NamespaceEnum.SysRole,selectedIds);
                  }
                  else if (e.key === 'exportAll') {
                    await handleExport(NamespaceEnum.SysRole);
                  }
                }}
                selectedKeys={[]}
              >
        ...
        params={params}
        request={params => {
                	        	                        	        	                        	        	                        	        	                        	                	                	        	                        	                	        	                        	        	                          return retrievePageList(params, NamespaceEnum.SysRole);
        }}
        columns={columns}
      ...
      <CreateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleAdd(value,NamespaceEnum.SysRole);
          setSubmitting(false);
          if (success) {
      ...
      <UpdateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleUpdate(value,NamespaceEnum.SysRole);
          setSubmitting(false);
          if (success) {
