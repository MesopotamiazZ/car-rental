...
import { SysRoleMenu } from './data.d';
...
import SysRoleMenuColumns from './Columns';
...
const SysRoleMenuTableList: React.FC<{}> = () => {
  ...
  const columns: ProColumns<SysRoleMenu>[] = [
    ...SysRoleMenuColumns,
    {
      title: '操作',
      dataIndex: 'option',
        ...
        <>
          ...
          <a
            onClick={() => {
              ...
              handleRemove(selectedIds,actionRef,NamespaceEnum.SysRoleMenu);
            }}
          >
            删除
          </a>
    ...
    <PageHeaderWrapper>
      <ProTable<SysRoleMenu>
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <OBtnUpload url={constructImportUrl(NamespaceEnum.SysRoleMenu)}/>,
            ...
            overlay={
              <Menu
                onClick={async e => {
                  ...
                  if (e.key === 'delete') {
                    await handleRemove(selectedIds,actionRef,NamespaceEnum.SysRoleMenu);
                    action.reload();
                  }
                  else if (e.key === 'export') {
                    await handleExport(NamespaceEnum.SysRoleMenu,selectedIds);
                  }
                  else if (e.key === 'exportAll') {
                    await handleExport(NamespaceEnum.SysRoleMenu);
                  }
                }}
                selectedKeys={[]}
              >
        ...
        params={params}
        request={params => {
                	                	        	                        	        	                        	                	        	                        	                	        	                        	        	                          return retrievePageList(params, NamespaceEnum.SysRoleMenu);
        }}
        columns={columns}
      ...
      <CreateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleAdd(value,NamespaceEnum.SysRoleMenu);
          setSubmitting(false);
          if (success) {
      ...
      <UpdateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleUpdate(value,NamespaceEnum.SysRoleMenu);
          setSubmitting(false);
          if (success) {
