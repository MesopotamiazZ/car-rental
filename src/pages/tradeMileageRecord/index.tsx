...
import { TradeMileageRecord } from './data.d';
...
import TradeMileageRecordColumns from './Columns';
...
const TradeMileageRecordTableList: React.FC<{}> = () => {
  ...
  const columns: ProColumns<TradeMileageRecord>[] = [
    ...TradeMileageRecordColumns,
    {
      title: '操作',
      dataIndex: 'option',
      valueType: 'option',
        ...
        <>
          ...
          <a
            onClick={() => {
              ...
              handleRemove(selectedIds,actionRef,NamespaceEnum.TradeMileageRecord);
            }}
          >
            删除
          </a>
    ...
    <PageHeaderWrapper>
      <ProTable<TradeMileageRecord>
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <OBtnUpload url={constructImportUrl(NamespaceEnum.TradeMileageRecord)}/>,
            ...
            overlay={
              <Menu
                onClick={async e => {
                  ...
                  if (e.key === 'delete') {
                    await handleRemove(selectedIds,actionRef,NamespaceEnum.TradeMileageRecord);
                    action.reload();
                  }
                  else if (e.key === 'export') {
                    await handleExport(NamespaceEnum.TradeMileageRecord,selectedIds);
                  }
                  else if (e.key === 'exportAll') {
                    await handleExport(NamespaceEnum.TradeMileageRecord);
                  }
                }}
                selectedKeys={[]}
              >
        ...
        params={params}
        request={params => {
                	                	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	                	        	                        	                	        	                        	        	                          return retrievePageList(params, NamespaceEnum.TradeMileageRecord);
        }}
        columns={columns}
      ...
      <CreateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleAdd(value,NamespaceEnum.TradeMileageRecord);
          setSubmitting(false);
          if (success) {
      ...
      <UpdateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleUpdate(value,NamespaceEnum.TradeMileageRecord);
          setSubmitting(false);
          if (success) {
...
export default TradeMileageRecordTableList;
