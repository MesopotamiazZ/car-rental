...
import { CarBasicFee } from './data.d';
...
import CarBasicFeeColumns from './Columns';
...
const CarBasicFeeTableList: React.FC<{}> = () => {
  ...
  const columns: ProColumns<CarBasicFee>[] = [
    ...CarBasicFeeColumns,
    {
      title: '操作',
      dataIndex: 'option',
        ...
        <>
          ...
          <a
            onClick={() => {
              ...
              handleRemove(selectedIds,actionRef,NamespaceEnum.CarBasicFee);
            }}
          >
            删除
          </a>
    ...
    <PageHeaderWrapper>
      <ProTable<CarBasicFee>
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <OBtnUpload url={constructImportUrl(NamespaceEnum.CarBasicFee)}/>,
            ...
            overlay={
              <Menu
                onClick={async e => {
                  ...
                  if (e.key === 'delete') {
                    await handleRemove(selectedIds,actionRef,NamespaceEnum.CarBasicFee);
                    action.reload();
                  }
                  else if (e.key === 'export') {
                    await handleExport(NamespaceEnum.CarBasicFee,selectedIds);
                  }
                  else if (e.key === 'exportAll') {
                    await handleExport(NamespaceEnum.CarBasicFee);
                  }
                }}
                selectedKeys={[]}
              >
        ...
        params={params}
        request={params => {
                	                	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	                	        	                        	                	        	                        	        	                        	        	                          return retrievePageList(params, NamespaceEnum.CarBasicFee);
        }}
        columns={columns}
      ...
      <CreateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleAdd(value,NamespaceEnum.CarBasicFee);
          setSubmitting(false);
          if (success) {
      ...
      <UpdateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleUpdate(value,NamespaceEnum.CarBasicFee);
          setSubmitting(false);
          if (success) {
...
export default CarBasicFeeTableList;
