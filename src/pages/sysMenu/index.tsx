...
import SysMenuColumns from './Columns';
...
const SysMenuTableList: React.FC<{}> = () => {
        ...
        <>
          ...
          <a
            onClick={() => {
              ...
              handleRemove(selectedIds,actionRef,NamespaceEnum.SysMenu);
            }}
          >
            删除
          </a>
    ...
    <PageHeaderWrapper>
      <ProTable<SysMenu>
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <OBtnUpload url={constructImportUrl(NamespaceEnum.SysMenu)}/>,
            ...
            overlay={
              <Menu
                onClick={async e => {
                  ...
                  if (e.key === 'delete') {
                    await handleRemove(selectedIds,actionRef,NamespaceEnum.SysMenu);
                    action.reload();
                  }
                  else if (e.key === 'export') {
                    await handleExport(NamespaceEnum.SysMenu,selectedIds);
                  }
                  else if (e.key === 'exportAll') {
                    await handleExport(NamespaceEnum.SysMenu);
                  }
                }}
                selectedKeys={[]}
              >
        ...
        params={params}
        request={params => {
                	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	        	                        	                	        	                        	                	        	                        	        	                          return retrievePageList(params, NamespaceEnum.SysMenu);
        }}
        columns={columns}
      ...
      <CreateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleAdd(value,NamespaceEnum.SysMenu);
          setSubmitting(false);
          if (success) {
      ...
      <UpdateForm
        onSubmit={async value => {
          setSubmitting(true)
          const success = await handleUpdate(value,NamespaceEnum.SysMenu);
          setSubmitting(false);
          if (success) {
...
export default SysMenuTableList;
