 ...
 * 模块主页面
 */
const UserTokenTableList: React.FC = () => {
  ...
  const columns: ProColumns<UserToken>[] = [
    ...UserTokenColumns
    ,
    {
      title: '操作',
      ...
      render: (_, record) => (
        <Space split={<Divider type="vertical" />} size={[0, 8]} wrap>
          ...
          ><EditOutlined /></a>
          <Divider type="vertical" />
          <a  title="删除" onClick={() => handleRemove([record.id],actionRef,NamespaceEnum.UserToken)}><DeleteOutlined /></a>
        </Space>
      ),
    },
  ];
  ...
  return (
    <PageHeaderWrapper>
    {
        isMobile() ? <NativeList
          ...
          handleRemove={(id: any, actionRefs: any) => handleRemove(id, actionRefs, NamespaceEnum.UserToken)} /> :
      <ProTable<UserToken>
        actionRef={actionRef}
        rowKey="id"
            ...
            overlay={
              <Menu
                onClick={async e => {
                  ...
                  if (e.key === 'remove') {
                    await handleRemove(selectedIds, actionRef, NamespaceEnum.UserToken);
                    action?.reload();
                  }
                }}
                selectedKeys={[]}
        ...
        request={requestParams => {
          return retrievePageList(requestParams, NamespaceEnum.UserToken);
        }}
        columns={columns}
      ...
      {createModalVisible && <CreateForm
        onSubmit={async (value:any) => {
          setSubmitting(true)
          const success = await handleAdd(value,NamespaceEnum.UserToken);
          setSubmitting(false);
          if (success) {
      ...
      {updateModalVisible && <UpdateForm
        onSubmit={async (value: any) => {
          setSubmitting(true)
          const success = await handleUpdate(value, NamespaceEnum.UserToken);
          setSubmitting(false);
          if (success) {
...
export default UserTokenTableList;
