...
import OssColumns from './Columns';
    ...
    ,
    {
      ...
      render: (_, record) => (
        <Space split={<Divider type="vertical" />} size={[0, 8]} wrap>
          ...
          ><EditOutlined /></a>
          <Divider type="vertical" />
          <a  title="删除" onClick={() => handleRemove([record.id],actionRef,NamespaceEnum.Oss)}><DeleteOutlined /></a>
        </Space>
      ),
    },
  ];
  ...
  return (
    <PageHeaderWrapper>
    {
        isMobile() ? <NativeList
          ...
          handleRemove={(id: any, actionRefs: any) => handleRemove(id, actionRefs, NamespaceEnum.Oss)} /> :
      <ProTable<Oss>
        actionRef={actionRef}
        rowKey="id"
            ...
            overlay={
              <Menu
                onClick={async e => {
                  ...
                  if (e.key === 'remove') {
                    await handleRemove(selectedIds, actionRef, NamespaceEnum.Oss);
                    action?.reload();
                  }
                }}
                selectedKeys={[]}
        ...
        request={requestParams => {
          return retrievePageList(requestParams, NamespaceEnum.Oss);
        }}
        columns={columns}
      ...
      {createModalVisible && <CreateForm
        onSubmit={async (value:any) => {
          setSubmitting(true)
          const success = await handleAdd(value,NamespaceEnum.Oss);
          setSubmitting(false);
          if (success) {
      ...
      {updateModalVisible && <UpdateForm
        onSubmit={async (value: any) => {
          setSubmitting(true)
          const success = await handleUpdate(value, NamespaceEnum.Oss);
          setSubmitting(false);
          if (success) {
